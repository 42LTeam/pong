...
export class AuthController {
    ...
    async doubleAuth(@Req() request, @Body() body : DoubleAuthDto){
        const user = await request.user;
        request.session.totp = authenticator.verify({token: body.token, secret: user.secretO2FA})
    ...
    async status(@Req() request) {
        ...
        return {user : {...ret, secretO2FA: Boolean(ret.secretO2FA)}, destination: !request.session.totp ? '2fa' : ''};
    }
