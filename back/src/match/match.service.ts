...
export class MatchService {
  ...
  async getCommonUserMatches(userId1: number, userId2: number): Promise<UserMatch[]> {
...
async getUserMatchStats(userId1: number, userId2: number): Promise<{ wins: number; losses: number }> {
  const userMatches1 = await this.prisma.userMatch.findMany({
      where: {
          userId: userId1,
      },
  ...
  for (const userMatch1 of userMatches1) {
      const matchingUserMatch2 = userMatches2.find(um => um.matchId === userMatch1.matchId);
