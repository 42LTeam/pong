...
export default class GameBall {
	...
	newBall() {
		...
		} while (this.MAX_BOUNCE_ANGLE <= Math.abs(this.angle)
	...
	update() {
		if (this.position.y - this.BALL_SEMI_SIZE <= 0
			|| 1 <= this.position.y + this.BALL_SEMI_SIZE
		) {
			this.velocity.y *= -1;
			this.angle *= -1;
		} else if (this.position.x <= this.BALL_SEMI_SIZE * 5
		...
		} else if (this.position.x >= 1 - this.BALL_SEMI_SIZE * 5
			...
			this.angle =
				...
				(this.MAX_BOUNCE_ANGLE /
					(this.game.players[0].PLAYER_SEMI_HEIGHT + this.BALL_SEMI_SIZE)) + Math.PI;
			this.updateSpeed(false);
		}
