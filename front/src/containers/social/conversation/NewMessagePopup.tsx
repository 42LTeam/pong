...
export default function NewMessagePopup({ position, clear }: Props) {
    ...
    const mapData = (current) => (
        <Friend key={"popupfriend-" + current.id} friend={current}
            ...
            <div className="align-left">
                <input readOnly
                    {...(checked.includes(current.username) ? { checked: true } : { checked: false })}
                    type="checkbox" />
            </div>
        </Friend>
    );
    ...
    const handlePopupSearch = async (event) => {
        ...
        if (!search) return getAllUsers({ notFriend: false, friendOnly: false })
            .then(response => setSuggestions(response.data.filter(current => current.id !== user.id)));
    ...
    if (!suggestions.length)
        getAllUsers({ notFriend: false, friendOnly: false })
            .then(response => setSuggestions(response.data.filter(current => current.id !== user.id)));
    ...
    return (
        <PopUp key={"newmessage-root"} position={position} clear={clear}>
            ...
            {checked.length === 0 ? (
                <Button key={"newmessage-button"} fill text="Select Friends First" clickable={false}></Button>
            ) : checked.length === 1 ? (
                <Button key={"newmessage-button"} fill handleClick={() => handleClickMP(true)} text="Creer un MP" clickable={true}></Button>
            ) : (
                <div style={{ display: 'flex', width: '100%', justifyContent: 'space-between' }}>
                    <Button key={"newmessage-button-public"} fill handleClick={() => handleClick(false)} text="Creer un channel public" clickable={true} style={{ flex: 1, marginRight: '10px' }}></Button>
                    <Button key={"newmessage-button-private"} fill handleClick={() => handleClick(true)} text="Creer un channel privé" clickable={true} style={{ flex: 1, marginLeft: '10px' }}></Button>
                </div>
            )}
        </PopUp>
    )
