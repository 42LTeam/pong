...
import { updateUserAvatar, updateUserUsername, uploadUserAvatar } from "../../api";
        ...
        if (file) {
            try {
                ...
                if (response.status === 201) {
                    console.log(`User avatar updated successfully. Path : ${response.data.path}`, response.data.path);
                    setAvatarUrl(response.data.path);
                } else {
                    console.error("Failed to update user avatar. Response status:", response.status);
