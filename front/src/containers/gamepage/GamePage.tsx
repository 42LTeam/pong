...
export default function GamePage() {
  ...
  const draw = (status, countdown) => {
    ...
    c2d.fillRect(
      (players.player0.x - ball.semiSize) * c2d.canvas.width,
      (players.player0.y - players.semiHeight) * c2d.canvas.height,
      ball.semiSize * 2 * c2d.canvas.width,
      players.semiHeight * 2 * c2d.canvas.height,
    );
    c2d.fillRect(
      (players.player1.x - ball.semiSize) * c2d.canvas.width,
    ...
    c2d.fillText(
      players.player0.score.toString(),
      c2d.canvas.width * 0.375,
      textPos,
    );
    c2d.textAlign = "right";
  ...
  useEffect(() => {
    if (canvas) socket.emit("join-game");
  }, [canvas]);
  return (
    <>
